...
COL_STATE_EVENT_RECORD  = ["record_t", "cpu_id", "appl_id", "task_id", "thread_id", "time_ini", "time_fi", "state", "event_t", "event_val"]
COL_COMM_RECORD         = ["record_t", "cpu_send_id", "ptask_send_id", "task_send_id", "thread_send_id", "lsend", "psend", "cpu_recv_id", "ptask_recv_id", "task_recv_id", "thread_recv_id", "lrecv", "precv", "size", "tag"]
...
def read_lines(file, bytes=None):
    ...
    logger.info(f"Has been read {'{:,.0f}'.format(size)} MB in {'{:.2f}'.format(elapsed)} seconds ({'{:.3f}'.format(size/elapsed)} MB/s)")
    return lines
    ...
    with open(file, "r") as f:
        ...
        for numline, line in enumerate(lines):
            ...
            if record_t == STATE_RECORD or record_t == EVENT_RECORD:
                df_state_event = df_state_event.append(pd.DataFrame(rows, columns=COL_STATE_EVENT_RECORD), ignore_index=True)
            elif record_t == COMM_RECORD:
